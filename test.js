...
/**
 * Enhanced JavaScript interpreter with support for:
 * - Array methods (push, pop, map, filter, forEach, etc.)
 * - String methods (length, toUpperCase, substring, etc.)
 * - Arrow functions
 * - Template literals
 */
class CodeInterpreter {
  ...
  handleVariableDeclaration(node) {
    node.declarations.forEach(declaration => {
      ...
      this.addStep({
        ...
        lineContent: this.getSourceLine(node),
        variables: { ...this.getCurrentScope() },
  ...
  handleAssignment(node) {
    ...
    if (node.left.type === 'MemberExpression') {
      ...
      this.addStep({
        ...
        lineContent: this.getSourceLine(node),
        variables: { ...this.getCurrentScope() },
        description: `Set ${node.left.object.name}[${property}] = ${this.formatValue(value)}`,
        type: 'assignment',
        consoleOutput: [...this.consoleOutput],
        callStack: [...this.callStack]
      });
  ...
  /**
   * Handle function calls (including array/string methods)
   */
  handleFunctionCall(node) {
    // Handle member expression calls (array/string methods, console.log)
    if (node.callee.type === 'MemberExpression') {
      const object = this.evaluateExpression(node.callee.object);
      const methodName = node.callee.property.name;

      // Handle console.log
      if (node.callee.object.name === 'console' && methodName === 'log') {
  ...
  handleArrayMethod(node, array, methodName) {
    ...
    switch (methodName) {
      case 'push':
        ...
        description = `Array.push(${args.map(a => this.formatValue(a)).join(', ')}) → length: ${result}`;
        break;
      ...
      case 'unshift':
        ...
        description = `Array.unshift(${args.map(a => this.formatValue(a)).join(', ')}) → length: ${result}`;
        break;

      case 'slice':
        result = array.slice(...args);
        description = `Array.slice(${args.join(', ')}) → ${this.formatValue(result)}`;
        break;

      case 'splice':
        result = array.splice(...args);
        description = `Array.splice(${args.join(', ')}) → removed: ${this.formatValue(result)}`;
        break;
      ...
      case 'includes':
        ...
        description = `Array.includes(${this.formatValue(args[0])}) → ${result}`;
        break;
  ...
  handleArrayIteratorMethod(array, callback, methodName) {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      const callbackResult = this.executeCallback(callback, [element, i, array]);
  ...
  handleArrayReduce(array, callback, initialValue) {
    ...
    for (let i = startIndex; i < array.length; i++) {
      accumulator = this.executeCallback(callback, [accumulator, array[i], i, array]);
    }
  ...
  handleStringMethod(node, string, methodName) {
    ...
    switch (methodName) {
      ...
      case 'split':
        ...
        description = `String.split('${args[0]}') → ${this.formatValue(result)}`;
        break;
  ...
  evaluateExpression(node) {
    ...
    switch (node.type) {
      ...
      case 'MemberExpression':
        ...
        if (property === 'length' && (typeof object === 'string' || Array.isArray(object))) {
  ...
  formatValue(value) {
    ...
    if (Array.isArray(value)) {
      if (value.length > 10) {
        return `[${value.slice(0, 10).map(v => this.formatValue(v)).join(', ')}, ... +${value.length - 10} more]`;
      }
      return `[${value.map(v => this.formatValue(v)).join(', ')}]`;
    }
    if (typeof value === 'object' && value !== null) {
      if (value.type === 'arrow-function') {
        return '(arrow function)';
      }
      const entries = Object.entries(value);
      if (entries.length > 3) {
        return `{${entries.slice(0, 3).map(([k, v]) => `${k}: ${this.formatValue(v)}`).join(', ')}, ...}`;
      }
